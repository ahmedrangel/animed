...
const added = computed(() => watchlist.value?.find(item => item.mediaId === Number(id)));
  ...
  watchlist.value = await Promise.all(watchlist.value.map(async (item) => {
    if (item.mediaId === Number(id)) {
      ...
      const updated = await $fetch("/api/watchlist", {
        method: "PATCH",
        body: { mediaId: item.mediaId, userId: item.userId, ...toUpdate }
      }).catch(() => null);
      if (!updated) {
              ...
              <div v-if="anime">
                <div id="details" class="pt-2 pb-4 d-flex align-items-start anime-row mx-0 flex-wrap px-0">
                  <div class="col-12 col-sm-12 col-md-4 col-lg-4 col-xl-3 px-0" style="max-width: 400px;">
                    ...
                    <ButtonComp v-if="loggedIn && !added && user" v-ripple="{ color: 'rgba(0,0,0,0.4)' }" class="bg-primary text-dark w-100" icon="ph:plus-bold" @click="addToWatchlist(user.id, anime.id)">
                      Add to Watchlist
                    </ButtonComp>
                    <div v-if="loggedIn && added">
                      <div class="input-group mb-2">
                        <label class="input-group-text bg-primary bg-secondary">Status</label>
                        <select v-model="watchlistData.status" class="form-select" :disabled="updating">
                      ...
                      <div class="input-group mb-2">
                        <span class="input-group-text bg-primary bg-secondary">Progress</span>
                        <input v-model="watchlistData.progress" type="number" class="form-control" placeholder="0" :disabled="updating">
                      ...
                      <div class="input-group mb-2">
                        <span class="input-group-text bg-primary bg-secondary">Score</span>
                        <select v-model="watchlistData.score" class="form-select" :disabled="updating">
                      ...
                      <div class="input-group mb-2">
                        <span class="input-group-text bg-primary bg-secondary">Started</span>
                        <input v-model="watchlistData.startedDate" type="date" class="form-control" :disabled="updating">
                      </div>
                      <div class="input-group mb-2">
                        <span class="input-group-text bg-primary bg-secondary">Finished</span>
                        <input v-model="watchlistData.finishedDate" type="date" :min="watchlistData.startedDate || undefined" class="form-control" :disabled="updating">
                      </div>
                      <ButtonComp v-ripple="{ color: 'rgba(0,0,0,0.4)' }" class="bg-primary text-dark w-100" icon="ph:floppy-disk-bold" :disabled="updateButtonDisabled" @click="updateWatchlist">
                        Update
                      </ButtonComp>
                    </div>
                  </div>
