...
const { params } = useRoute("c-top-rated-slug");
const { slug } = params;
...
const { data: result }: { data: Ref<AnimeList> } = await useFetch("/api/explore/rated?slug=" + slug);
...
useSeoMeta({
  ...
  ogUrl: SITE.url + `/c/top-rated/${slug}`,
...
useHead({
  link: [{ rel: "canonical", href: SITE.url + `/c/top-rated/${slug}` }]
});
</script>
