...
<template>
  <div class="row m-0 flex-lg-row overflow-hidden min-vh-100">
    <div class="sidebar p-0 bg-dark d-lg-block d-none border-end col-1">
      <ul id="menu" class="nav flex-column justify-content-evenly align-items-center min-vh-100 position-fixed z-3">
        <li v-for="(tab, i) of tabs" :key="i" class="nav-item w-100 d-flex flex-grow-1">
          <NuxtLink v-ripple :to="tab.route" class="nav-link p-0 text-body d-flex justify-content-center align-items-center w-100" :title="tab.name">
    ...
    <slot />
    <nav class="navbar bg-dark d-lg-none d-block border-top w-100 position-fixed z-3 bottom-0 p-0">
      <ul id="menu" class="nav align-items-center justify-content-evenly">
        <li v-for="(tab, i) of tabs" :key="i" class="nav-item flex-fill">
          <NuxtLink v-ripple :to="tab.route" class="nav-link py-3 px-0 text-body" :title="tab.name">
