...
<template>
  <div class="position-relative" data-aos="fade-in">
    ...
    <div v-ripple class="image overflow-hidden mb-2 w-100 position-relative" @mouseover="cardHovered = true" @mouseleave="cardHovered = false">
      ...
      <Icon v-if="cardHovered && loggedIn && !added && user?.id" name="ph:plus-bold" class="bookmark position-absolute bottom-0 end-0 m-3 z-1 h4 text-white" @click="addToWatchlist(props.data.id, fixSlug(data.title.romaji))" />
